...
def main():
    ...
    parser.add_argument("--sim_file", default="sim_test0.root", help="Path to simulation file")
    ...
    def get_derivatives_vec(x, y, z, px, py, pz):
    ...
    with uproot.open(args.sim_file) as sim_file:
    ...
    # to avoid the jagged array problem before putting it into Pandas.
    # Create a boolean mask to ONLY look at hits belonging to the primary muon (TrackID == 0)

    is_primary_track = ak_muons["strawtubesPoint.fTrackID"] == 0

    df_muons = pd.DataFrame({
        "strawtubesPoint.fX": ak.firsts(ak_muons["strawtubesPoint.fX"][is_primary_track]),
        "strawtubesPoint.fY": ak.firsts(ak_muons["strawtubesPoint.fY"][is_primary_track]),
        "strawtubesPoint.fZ": ak.firsts(ak_muons["strawtubesPoint.fZ"][is_primary_track]),
        "strawtubesPoint.fPx": ak.firsts(ak_muons["strawtubesPoint.fPx"][is_primary_track]),
        "strawtubesPoint.fPy": ak.firsts(ak_muons["strawtubesPoint.fPy"][is_primary_track]),
        "strawtubesPoint.fPz": ak.firsts(ak_muons["strawtubesPoint.fPz"][is_primary_track]),
        # MCTrack properties are usually single values per event, but ak.firsts handles them cleanly
        "MCTrack.fStartX": ak.firsts(ak_muons["MCTrack.fStartX"]),
        "MCTrack.fStartY": ak.firsts(ak_muons["MCTrack.fStartY"])
    })

    df_primary = df_muons.dropna().copy()

    # ==========================================
    # 4. PROPAGATE MUONS BACKWARDS TO UBT
    # ==========================================
    print(f"Propagating {len(df_primary)} muons backwards...")

    # Prepare lists to hold our final UBT coordinates
    ubt_x_hits = []
    ubt_y_hits = []
    ubt_z_hits = []

    # Grab the initial state at the SST
    x = df_primary["strawtubesPoint.fX"].values.copy()
    y = df_primary["strawtubesPoint.fY"].values.copy()
    z = df_primary["strawtubesPoint.fZ"].values.copy()
    px = df_primary["strawtubesPoint.fPx"].values.copy()
    py = df_primary["strawtubesPoint.fPy"].values.copy()
    pz = df_primary["strawtubesPoint.fPz"].values.copy()
    p_total = np.sqrt(px**2 + py**2 + pz**2)
    q = -1.0

    # Define the target Z position and step size for the RK4 integration
    target_z = args.UBT_z
    dz = args.step_size

    # Propagate the muons backwards until they reach the UBT
    print(f"Propagating {len(df_primary)} muons...")
    while np.any(z > args.UBT_z):
        ...
        # Stage 1 (initial)
        k1 = dz*get_derivatives_vec(x, y, z, px, py, pz)
    ...
    df_primary["UBT_X_mm"] = (x - df_primary["MCTrack.fStartX"].values) * 10
    df_primary["UBT_Y_mm"] = (y - df_primary["MCTrack.fStartY"].values) * 10
    df_primary["UBT_Z_cm"] = (z)

    print("Propagation Complete! You can now plot the 'UBT_X_mm' and 'UBT_Y_mm' and 'UBT_Z_cm' columns.")
    ...
    plot_hist(axes[0], df_primary["UBT_X_mm"], "Distribution at UBT (X)", " X [mm]", 'blue')
    plot_hist(axes[1], df_primary["UBT_Y_mm"], "Distribution at UBT (Y)", " Y [mm]", 'red')

    # XY Correlation (2D Histogram)
    h = axes[2].hist2d(df_primary["UBT_X_mm"], df_primary["UBT_Y_mm"], bins=100, cmap='viridis')
    ...
    plot_hist(axes2[0], df_primary["strawtubesPoint.fX"], "Distribution at SST (X)", " X [mm]", 'blue')
    plot_hist(axes2[1], df_primary["strawtubesPoint.fY"], "Distribution at SST (Y)", " Y [mm]", 'red')

    # XY Correlation (2D Histogram)
    h = axes2[2].hist2d(df_primary["strawtubesPoint.fX"], df_primary["strawtubesPoint.fY"], bins=100, cmap='viridis')
    ...
    plot_hist(axes4[0], df_primary["UBT_Z_cm"], "Distribution at UBT (Z)", " Z [cm]", 'green')
    plot_hist(axes4[1], df_primary["strawtubesPoint.fZ"], "Distribution at SST (Z)", " Z [cm]", 'orange')
